#!/usr/bin/env node
/**
 * Приймач деплою бекенду. Крутиться на сервері з-під systemd --user, робоча
 * копія лежить поруч із чекаутом API. Цей файл у репозиторії є джерелом
 * правди; після зміни його треба покласти на місце руками.
 */
...
if (!SECRET) { console.error('FATAL: WEBHOOK_SECRET env required'); process.exit(1); }
// За замовчуванням — backend/ цього ж чекауту; переозначається змінною REPO_DIR.
const REPO_DIR = process.env.REPO_DIR || path.resolve(__dirname, '..', 'backend');
const BRANCH = 'main';
const PUBLIC_STATIC_DIR = process.env.PUBLIC_STATIC_DIR || '/var/www/navernyborshchu/photos/staticfiles';
